...
else
{
    ...
    $smarty->assign('hot_goods',       get_category_recommend_goods('hot', $children, $brand, $price_min, $price_max, $ext,6));
		...
		$sql = "SELECT cat_id,cat_name FROM ".$GLOBALS['ecs']->table('category') . "WHERE parent_id = '$cat_id' AND is_show = 1 ORDER BY sort_order";
		$res = $GLOBALS['db']->getAll($sql);
		foreach($res AS $v)
		{
			$str[$v['cat_id']]['goods']=assign_cat_goods($v['cat_id'], $num = 10,'cate');
			
			$mysql = "SELECT cat_id,cat_name FROM ".$GLOBALS['ecs']->table('category') . "WHERE parent_id = '".$v['cat_id']."' AND is_show = 1";
