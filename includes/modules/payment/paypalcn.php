...
if (isset($set_modules) && $set_modules == TRUE)
{
    ...
    /* 网址 */
    $modules[$i]['website'] = 'http://www.paypal.com/cn';
    ...
    $modules[$i]['config']  = array(
        array('name' => 'paypalcn_account', 'type' => 'text', 'value' => ''),
    );
...
class paypalcn
{
    ...
    function get_code($order, $payment)
    {
        ...
        $data_pay_account   = $payment['paypalcn_account'];
        ...
        $def_url  = '<br /><form style="text-align:center;" action="https://www.paypal.com/cgi-bin/webscr" method="post">' .   // 不能省略
            ...
            "<input type='hidden' name='no_note' value='0'>" .                                  // 付款说明
            "<input type='hidden' name='currency_code' value='CNY'>" .                          // 货币：人民币
            "<input type='hidden' name='notify_url' value='$data_notify_url'>" .
            "<input type='hidden' name='rm' value='2'>" .
            "<input type='hidden' name='cancel_return' value='$cancel_return'>" .
            "<input type='submit' value='" . $GLOBALS['_LANG']['paypalcn_button'] . "'>" .                      // 按钮
            "</form><br />";
    ...
    function respond()
    {
        $payment        = get_payment('paypalcn');
        $merchant_id    = $payment['paypalcn_account'];               ///获取商户编号
