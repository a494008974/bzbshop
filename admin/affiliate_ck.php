...
if ($_REQUEST ['act'] == 'list') {
	...
	if (! empty ( $_GET ['auid'] )) {
		$smarty->assign ( 'action_link', array (
				...
				'href' => "users.php?act=edit&id=$_GET[auid]" 
		) );
	}
	assign_query_info ();
...
elseif ($_REQUEST ['act'] == 'query') {
	...
	make_json_result ( $smarty->fetch ( 'affiliate_ck_list.htm' ), '', array (
			'filter' => $logdb ['filter'],
			'page_count' => $logdb ['page_count'] 
	) );
} /*
   * 取消分成，不再能对该订单进行分成
   */
elseif ($_REQUEST ['act'] == 'del') {
	...
	$links [] = array (
			'text' => $_LANG ['affiliate_ck'],
			'href' => 'affiliate_ck.php?act=list' 
	);
...
elseif ($_REQUEST ['act'] == 'separate') {
	...
	$oid = ( int ) $_REQUEST ['oid'];
	if($oid == 'all'){
		affiliate_separate_all($db,$affiliate,$separate_by);
	}else{
		...
		affiliate_separate($db,$row,$oid,$affiliate,$separate_by);
	}
	$links [] = array (
			'text' => $_LANG ['affiliate_ck'],
			'href' => 'affiliate_ck.php?act=list' 
...
function affiliate_separate($db,$row,$oid,$affiliate,$separate_by){
	...
	if (empty ( $row ['is_separate'] )) {
		...
		$integral = integral_to_give ( array (
				'order_id' => $oid,
				'extension_code' => ''
		) );
...
function get_affiliate_ck() {
	...
	if (! empty ( $affiliate ['on'] )) {
		if (empty ( $separate_by )) {
			...
			/*
			 * SQL解释： 列出同时满足以下条件的订单分成情况： 1、有效订单o.user_id > 0 2、满足以下情况之一： a.有用户注册上线的未分成订单 u.parent_id > 0 AND o.is_separate = 0 b.已分成订单 o.is_separate > 0
			 */
		} else {
			// 推荐订单分成
			...
			/*
			 * SQL解释： 列出同时满足以下条件的订单分成情况： 1、有效订单o.user_id > 0 2、满足以下情况之一： a.有订单推荐上线的未分成订单 o.parent_id > 0 AND o.is_separate = 0 b.已分成订单 o.is_separate > 0
			 */
		}
	} else {
		// 关闭
