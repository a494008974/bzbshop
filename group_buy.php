    ...
    if (!$smarty->is_cached('group_buy_list.dwt', $cache_id))
    {
        if ($count > 0)
        {
            ...
            /* 设置分页链接 */
				...
				$GLOBALS['_CFG']['page_style']=1;  //zhong改
...
elseif ($_REQUEST['act'] == 'view')
{
	...
	//dqy add start 2011-8-25
	$smarty->assign('group_buy_id',$group_buy_id);
	//dqy add end 2011-8-25
	...
	$smarty->assign('group_list',group_buy_info1($group_buy_id));
...
//    }
    ...
    if (!$smarty->is_cached('group_buy_goods.dwt', $cache_id))
    {
        ...
        $properties = get_goods_properties($goods_id);
			...
			$smarty->assign('properties',    $properties['pro']);   //仲改
...
elseif ($_REQUEST['act'] == 'buy')
{
	...
	$group_buy_id = $_REQUEST['group_buy_id'];
    if ($group_buy_id <= 0)
    {
        ecs_header("Location: ./\n");
        exit;
...
function group_buy_list($size, $page)
{
    ...
    $gb_list = array();
    $now = gmtime();
    $sql = "SELECT b.*, IFNULL(g.goods_thumb, '') AS goods_thumb,g.*, b.act_id AS group_buy_id, ".
    ...
    while ($group_buy = $GLOBALS['db']->fetchRow($res))
    {
		...
		//dqy add start 2011-8-24
		$group_buy['cur_price']  = $price_ladder[0]['price'];    // 初始化	
		if($group_buy['shop_price'] != 0)
		{
		   $group_buy['zhekou'] = number_format(intval($group_buy['cur_price'])/intval($group_buy['market_price'])*10,1)+"";
		}
		$group_buy['jiesheng'] = $group_buy['market_price'] - $group_buy['cur_price'];
 		//dqy add end 2011-8-24
...
function group_buy_info1($group_buy_id)
{
    ...
    $sql = "SELECT b.*,g.*, b.act_id AS group_buy_id, b.act_desc AS group_buy_desc, b.start_time AS start_date, b.end_time AS end_date " .
            ...
            "AND act_type = '" . GAT_GROUP_BUY . "' and is_finished =0 ";
